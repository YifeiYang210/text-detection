...
class SkewDetect:
    ...
    def display_houghline(self, h, theta, d):
        image = io.imread(self.input_file, as_gray=True)
        io.imshow(image)
        ax2 = plt.gca()
    ...
    def determine_skew(self, img_file):
        """grey->canny->hough to get angle"""
        img = io.imread(img_file, as_grey=True)
        edges = canny(img, sigma=self.sigma)
        io.imshow(edges)
        h, a, d = hough_line(edges)  # get hough space
        _, ap, _ = hough_line_peaks(h, a, d, num_peaks=self.num_peaks)  # get peaks(alpha,theta) from hough space
        self.display_houghline(h, a, d)
        # print("angle(-1.57to+1.57)列表：", ap)
        if len(ap) == 0:
            return {"Image File": img_file, "Message": "Bad Quality"}

        absolute_deviations = [self.calculate_deviation(k) for k in ap]  # list:[(angle-pi/4),]
        average_deviation = np.mean(np.rad2deg(absolute_deviations))
        ap_deg = [np.rad2deg(x) for x in ap]
        print("degree(-180to+180)列表：", ap_deg)
        ...
        for j in range(len(angles)):  # seperate by 45 to choose max line region to rotate
            l = len(angles[j])
            if l > lmax:
                lmax = l
                maxi = j
